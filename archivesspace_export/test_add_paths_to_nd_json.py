...
class Test(unittest.TestCase):
    ...
    def test_1_test_creating_json_from_xml(self):
        ...
        config['local'] = True
        add_files_to_nd_json_class = AddPathsToNdJson(config)
        nd_json_with_files = add_files_to_nd_json_class.add_paths(nd_json)
        # with open(local_folder + 'test/MSNEA8011_EAD_with_files_and_paths.json', 'w') as f:
        #     json.dump(nd_json_with_files, f, indent=2)
        with open(local_folder + 'test/MSNEA8011_EAD_with_files_and_paths.json', 'r') as input_source:
